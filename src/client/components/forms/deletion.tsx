...
function EntityDeletionForm({entity}: Props): React.JSX.Element {
	const [states, updateStates] = React.useState<States>({
		error: null,
		note: null,
		waiting: false
	});

	const entityUrl = `/${_kebabCase(entity.type)}/${entity.bbid}`;
	const deleteUrl = `${entityUrl}/delete/handler`;

	let errorComponent = null;
	if (states.error) {
		errorComponent =
			<Alert variant="danger">{states.error}</Alert>;
	}

	const loadingComponent = states.waiting ? <LoadingSpinner/> : null;

	const hasNote = states.note && states.note.length;
	...
	function handleNoteChange(event) {
		updateStates((prevStates: States) => ({
			...prevStates,
	...
	function handleSubmit(event) {
		...
		if (!states.note || !states.note.length) {
			updateStates((prevStates: States) => ({
				...prevStates,
